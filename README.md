...
This repository contains the results of technical challenge. Here I will outline key features and share the instructions for testing.

### **Key Features**

- Polygon area calculation (in square meters)
- Report generation (at any point in workflow execution, multiple reports can be generated by design)
- Interdependent Tasks
- Workflow final results aggregation
- New API endpoints for workflow status and results
- (Additional) Running analysis for different workflows, with workflow name provided by the API user
- (Additional) Endpoint inputs validation
- (Additional) Enhanced logging
- (Additional) Randomly failing email job to demonstrate error handling
- (Additional) Several smaller enhancements and refactorings (http statuses, job context, error information storing, etc.)

### **How to Test**

For your convenience I've added several fixtures and workflow examples that you could test these features with. All fixtures can be found in the `fixtures` directory. Take one and simply use as a `body` for your `POST /analysis` request.
